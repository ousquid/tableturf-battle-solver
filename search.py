...
class Card:
    def __init__(self, number: int, name: str, pattern: np.ndarray, ink_spaces: int, sp_attack_cost: int):
        self.number = number
        self.name = name
    ...
    def load_text(path: str):
        ...
        max_width = max([len(l.rstrip()) for l in lines])
...
class Map():
    ...
    def load_text(path: str):
        ...
        number = int(os.path.splitext(os.path.basename(path))[0])
        name = "StraightStreet" 
        return Map(number, name, pattern)

def fill_eval(map: Map) -> int:
    pass

def can_be_put(map: Map, card: Card) -> bool:
    # マップからはみ出ていないか
    # 他のカードと重ならないか
    pass
...
if __name__ == "__main__":
    map = Map.load_text("maps/01.txt")
    cards = Card.load_dir("cards/*.txt")
    print(cards)

    solver = Solver.new()
    ans = solver.search(map, cards)
